...
test.describe('organisations', () => {
	...
	test('an admin creates an organisation, issues a token for it, and manages its members', async ({ page }) => {
		...
		await expect(
			page.getByRole('link', { name: `Open ${org}` }),
		).toBeVisible();
		...
		const secret =
			(await page.locator('.secret-value').textContent())?.trim() ?? '';
		...
		await expect(
			page.getByRole('heading', { level: 1, name: org }),
		).toBeVisible();
		await expect(
		...
		await expect(
			aliceRow.getByRole('button', { name: 'remove' }),
		).toHaveCount(0);
		...
		await expect(
			page.getByText(`${member} is now write in ${org}.`),
		).toBeVisible();
		...
		await expect(
			page.getByText(`${member} removed from ${org}.`),
		).toBeVisible();
		await expect(
			page.getByRole('row', { name: new RegExp(member) }),
		).toHaveCount(0);
